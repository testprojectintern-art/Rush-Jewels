...
import {
    useWatchAgingStock,
    useWatchBrandProfitability,
    useWatchAovBundles,
    useWatchSeasonality
} from '../../features/reports/useReports';

export default function WatchAnalyticsPage() {
    const navigate = useNavigate();
    const [activeTab, setActiveTab] = useState('aging');

    // Fetch queries
    const { data: agingRes, isLoading: loadingAging } = useWatchAgingStock();
    const { data: brandsRes, isLoading: loadingBrands } = useWatchBrandProfitability();
    const { data: bundlesRes, isLoading: loadingBundles } = useWatchAovBundles();
    const { data: seasonalRes, isLoading: loadingSeasonal } = useWatchSeasonality();
    ...
    return (
        <div>
            <PageHeader
                title="Watch & Retail Analytics"
                description="Specialized retail metrics and intelligence for watch inventory"
                actions={
                                ...
                                <div className="space-y-4 lg:col-span-1">
                                    <Card className="p-5 flex flex-col justify-between">
                                        ...
                                        <p className="text-[10px] text-gray-400 mt-4">
                                            Calculated based on {bundlesData.totalOrders} total orders. Higher AOV can be driven by styling bundle recommendations (e.g. watch + box + extra straps).
                                        </p>
                                    </Card>
