...
/**
 * GET /api/reports/jewelry/aging-stock
...
export const getInventoryAging = asyncHandler(async (req, res) => {
    ...
    const matchStage = { 'quantities.onHand': { $gt: 0 } };
    if (allowedWhIds) {
...
/**
 * GET /api/reports/jewelry/brand-profitability
 * Retrieve Net Profit Margin breakdown per Jewelry Brand (Pareto Analysis)
 */
export const getBrandProfitability = asyncHandler(async (req, res) => {
    const portalHeader = req.headers['x-portal-context'] || 'main';
    const data = await Invoice.aggregate([
        { $match: { status: { $ne: 'cancelled' }, deletedAt: null, ...getPortalFilter(portalHeader) } },
        { $unwind: '$items' },
        {
            $lookup: {
                from: 'products',
...
/**
 * GET /api/reports/jewelry/aov-bundles
...
export const getAovAndBundles = asyncHandler(async (req, res) => {
    const portalHeader = req.headers['x-portal-context'] || 'main';
    const invoices = await Invoice.find({ status: { $ne: 'cancelled' }, deletedAt: null, ...getPortalFilter(portalHeader) });
...
/**
 * GET /api/reports/jewelry/seasonality
